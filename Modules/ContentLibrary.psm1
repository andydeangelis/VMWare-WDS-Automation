...
#>
	param (
		...
		[String]$LibraryName,
		[Parameter(Mandatory = $false)]
		[Switch]$JSONPersistenceEnabled,
		[Parameter(Mandatory = $false)]
		[Switch]$JSONPersistenceDisabled
	)
...
#>
	param (
		...
		[String]$LibraryName,
		[Parameter(Mandatory = $false)]
		[String]$FolderName = "Workloads",
		[Parameter(Mandatory = $false)]
		[String]$ResourcePoolName = "Compute-ResourcePool"
	)
...
#>
	...
	$vmHostId = ((Get-VMHost -Name $VMHost).ExtensionData.MoRef).Value
	...
	$vmtxDeploySpec.placement.host = $vmHostId
...
#>
	param (
		...
		[Parameter(Mandatory = $true)]
		[String]$SubscriptionThumbprint,
		[Parameter(Mandatory = $false)]
		[Switch]$OnDemand,
		[Parameter(Mandatory = $false)]
		[Switch]$AutomaticSync
	)
	...
	if ($datastore)
	{
		...
		if ($OnDemand) { $OnDemandFlag = $true }
		else { $OnDemandFlag = $false }
		if ($AutomaticSync) { $AutomaticSyncFlag = $true }
		else { $AutomaticSyncFlag = $false }
